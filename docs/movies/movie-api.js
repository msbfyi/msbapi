...
class MovieAPI {
  ...
  async getThisYearActivity() {
    ...
    const thisYearWatches = watches.watches.filter(watch => 
      new Date(watch.watched_at).getFullYear() === currentYear
    );
  ...
  async getWatchingStreaks() {
    ...
    const watchDates = watches.watches.map(w => 
      new Date(w.watched_at).toDateString()
    );
  ...
  async getMoviesByRating(minRating, maxRating = 5) {
    ...
    const filteredWatches = watches.watches.filter(watch => 
      watch.personal_rating >= minRating && watch.personal_rating <= maxRating
    );
...
class MovieWebsite {
  ...
  generateRecentWatchesHTML(watches, maxItems = 5) {
    ...
    return watches.slice(0, maxItems).map(watch => `
