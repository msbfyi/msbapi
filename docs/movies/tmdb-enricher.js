...
class TMDBEnricher {
  ...
  async processBatch(movies, batchIndex) {
    ...
    this.log(`Processing batch ${batchIndex + 1}: ${batchStart + 1}-${batchEnd} of ${movies.length}`);
  ...
  async run() {
    ...
    try {
      ...
      if (this.stats.moviesNotFound > 0) {
        this.log(`📝 ${this.stats.moviesNotFound} movies not found on TMDB (check titles/years)`, 'info');
      }
...
async function main() {
  ...
  for (const field of requiredFields) {
    if (!CONFIG[field]) {
      const envVar = field === 'tmdbApiKey' ? 'TMDB_API_KEY' :
                    field === 'supabaseUrl' ? 'SUPABASE_URL' :
                    field === 'supabaseKey' ? 'SUPABASE_SERVICE_KEY' : field;
