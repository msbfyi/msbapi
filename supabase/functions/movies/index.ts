...
function extractTMDBData(tmdbMovie, tmdbDetails = null) {
  ...
  return Object.fromEntries(
    Object.entries(enrichedData).filter(([key, value]) => 
...
async function upsertMovie(movieData) {
  ...
  const enrichmentStatus = Object.keys(tmdbData).length > 0 
...
async function handleEchoFeedWebhook(req, corsHeaders) {
  try {
    ...
    if (payload.item) {
      ...
      throw new Error('No valid feed item found in webhook payload. Expected either payload.item or direct payload with id/title fields');
    }
...
async function extractMovieData(feedItem) {
  ...
  console.log(`Initial extraction - raw title: "${title}", link: "${link}", has_trakt_id: ${!!movieData.externalIds.trakt_id}`);
  ...
  console.log(`Final extracted data - title: "${extractedData.title}", year: ${extractedData.year}, trakt_id: ${extractedData.externalIds.trakt_id}`);
...
async function extractTraktData(feedItem, movieData) {
  ...
  console.log(`Trakt extraction result - title: "${movieData.title}", year: ${movieData.year}, review length: ${movieData.review?.length || 0}`);
...
async function getWatches(req, corsHeaders) {
  ...
  return new Response(
    JSON.stringify({ count: data.length, watches: data }),
    { 
      status: 200, 
      headers: { 
        ...corsHeaders,
...
async function handleCleanup(req, corsHeaders) {
  try {
    ...
    console.log(`Deleted ${deletedWatches?.length || 0} watches and ${deletedMovies?.length || 0} movies`);
...
async function getMovieStats(corsHeaders) {
  ...
  const { data: watchCount } = await supabase
    .from('movie_watches')
    .select('id', { count: 'exact' });
  ...
  const avgRatingValue = avgRating.length > 0 
  ...
  return new Response(
    JSON.stringify({
      ...
      poster_coverage: movieCount?.length > 0 
