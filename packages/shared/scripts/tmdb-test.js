...
async function testSupabaseConnection() {
  ...
  try {
    ...
    const response2 = await fetch(
      `${url}?select=id,title,year,poster_url&poster_url=is.null&limit=10`,
      {
        method: 'GET',
        headers,
      }
    ...
    console.log(
      `Response type: ${Array.isArray(moviesWithoutPosters) ? 'array' : typeof moviesWithoutPosters}`
    )
    ...
    console.log(
      `Movies needing enrichment: ${(totalMovies?.length || 0) - (moviesWithPosters?.length || 0)}`
    )
...
async function testTMDBConnection() {
  ...
  try {
    ...
    const searchResponse = await fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=${CONFIG.tmdbApiKey}&query=matrix`
    )
